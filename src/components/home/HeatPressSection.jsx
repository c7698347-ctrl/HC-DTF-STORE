...
import { ArrowRight, Eye, ShieldCheck, Zap, Flame, Check } from 'lucide-react';
import { useStore } from '@/context/StoreContext';

export default function HeatPressSection() {
  const { products = [], settings = {} } = useStore();

  if (settings.heatPressSectionEnabled === false) return null;

  const heatPressProducts = (products || []).filter(
    (p) => p.categoryId === 'cat-heatpress' || p.category === 'HEAT PRESS MACHINES'
  );

  const getWhatsAppBuyUrl = (modelName) => {
    const is1624 = modelName.includes('16×24') || modelName.includes('16x24');
    ...
    <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
      <div className="bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-white rounded-3xl p-8 sm:p-12 shadow-2xl border border-slate-800 space-y-10 relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-end justify-between gap-4 border-b border-slate-800 pb-6 relative z-10">
          <div className="space-y-2 max-w-2xl">
            ...
            <h2 className="text-3xl sm:text-5xl font-black text-white tracking-tight leading-tight">
              {settings.heatPressTitle || '🔥 JUKE HEAT PRESS MACHINES'}
            </h2>
            <p className="text-xs sm:text-sm text-slate-300 font-medium">
              {settings.heatPressSubtitle || 'Professional Heavy Duty Heat Press Machines for DTF Printing'}
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
          {heatPressProducts.map((prod) => {
            const waUrl = getWhatsAppBuyUrl(prod.name);
            ...
            return (
              ...
              >
                ...
                {/* Top Image Preview with Studio Lighting */}
                <div className="relative aspect-[16/10] bg-white rounded-2xl overflow-hidden border border-slate-700/60 p-4 flex items-center justify-center shadow-inner">
                  <img
                    src={prod.images?.[0] || '/images/juke_heat_press_16x24.png'}
                    alt={prod.name}
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-[11px] font-semibold text-slate-300 bg-slate-950/80 p-3 rounded-2xl border border-slate-800">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span>Size: {prod.specifications?.size || 'Heavy Duty Platen'}</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <Check size={14} className="text-emerald-400 shrink-0" />
                      <span>Voltage: 220V / 50Hz</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <Check size={14} className="text-emerald-400 shrink-0" />
                    ...
                    <div className="flex items-center gap-1.5">
                      <Check size={14} className="text-emerald-400 shrink-0" />
                      <span>1 Year Tech Warranty</span>
                    </div>
                  </div>

                  {/* Price Tag */}
                  <div className="flex items-baseline gap-2 pt-1">
                    <span className="text-3xl font-black text-white">₹{prod.price?.toLocaleString()}</span>
                    <span className="text-xs text-slate-400 font-bold uppercase tracking-wider">Commercial Net Price</span>
                  </div>
                </div>

                {/* Action Buttons: Buy Now (WhatsApp) & View Details */}
